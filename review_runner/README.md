...
1. `.github/workflows/ci.yml` runs every deterministic Pull Request check: branch naming and the gateway workflow/reviewer tests.
2. `.github/workflows/ai-review-after-ci.yml` runs from the default branch after `PR Quality` completes. Its job exists only when the completed run is a successful Pull Request run with resolved PR metadata.
3. `.github/workflows/reusable-ai-review.yml` checks out the exact base SHA, sets up Python, retrieves the PR diff through the GitHub API, and invokes `python -m review_runner.github_review`.
...
3. Failing branch validation or tests leaves the AI workflow unstarted and does not consume provider quota.
